...
def shops_grocery(request, shopname, shop_location):
    ...
    return render(request, 'groceries.html',
                  {'groceries': groceries, 'shop_name': shop.shop, 'shop_location': shop_location, 'dic': dic,
                   'shop': shop})
...
def cart_view(request, shopname, shop_location):
    ...
    try:
        ...
        return render(request, 'cart.html',
                      {'cart': {}, 'shop_name': shop_name, 'shop': shop, 'shop_location': shop_location,
                       'profile': profile})
    # form = DeliveryLocationForm()
    return render(request, 'cart.html',
                  {'cart': dic, 'profile': profile, 'shop_name': shop_name, 'shop_location': shop_location,
                   'shop': shop, })
...
def checkout(request, shopname, location_id, shop_location):
    ...
    order.save()
    with open(file_path3) as order_file:
...
def pastOrders(request):
    ...
    for order in orders:
        ...
        if json_dict['cancelled_by_user'].lower() == "yes":
            ...
            status = "cancelled by shop"
        elif json_dict['delivered'].lower() == "yes":
            status = "delivered"
        elif order.completed is True:
            status = "packed"
    ...
    order_list1 = []
    if user.shop_owner:
        shop = Shop.objects.get(shop_user=id)
        orders1 = Order.objects.filter(shop=shop.id)
        c = 0
...
def delivered_entries(request, order_num):
    ...
    for pro in ordered_prod:
        ...
        prod = {'index': c, 'prod_key': pro, 'ordered_prod': ordered_prod[pro],
                'amount': ordered_prod[pro]['Quantity'] * ordered_prod[pro]['SP']}
        products.append(prod)

    return render(request, 'delivered_orders.html', {'num': order_num + 1, 'product': products})
