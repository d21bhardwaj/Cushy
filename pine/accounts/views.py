...
def pg_view(request, room_id, image_id):
    ...
    try:
        ...
        return render(request, 'my_pg_detail.html',
                      {'rooms': rooms, 'images': images, 'hide': hide, 'seller': profile, "pg": 'kyahaiyeh'})
...
def room_view(request, room_id, image_id):
    ...
    try:
        ...
        return render(request, 'my_room_detail.html',
                      {'rooms': rooms, 'images': images, 'hide': hide, 'seller': profile, "body": "room"})
    ...
    if (extra_form != 0):
        ...
        if request.user.is_authenticated and request.user.id == user.id:
            ...
            return render(request, 'images_update.html',
                          {'rooms': rooms, 'imageform': imageform, 'images': images, "page": "room"})
        else:
            raise PermissionDenied
...
def pg_image(request, room_id):
    ...
    if (extra_form != 0):
        ...
        if request.user.is_authenticated and request.user.id == user.id:
            ...
            return render(request, 'images_update.html',
                          {'rooms': rooms, 'imageform': imageform, 'images': images, "page": "pg"})
        else:
            raise PermissionDenied
